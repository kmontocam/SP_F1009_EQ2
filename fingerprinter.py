...
def fingerprinter(audio_freq, audio, impact):
    freq_times = list()
    freq_highs = list()

    for down, up in zip(range(0, len(audio), HAMMING), range(0, len(audio), HAMMING)[1:]):
        
        audio_hammed = audio[down:up]

        fourier_coeff = np.abs(fft(audio_hammed)) # Apply FFT on interval
        fourier_coeff = fourier_coeff[:len(fourier_coeff)//2] # Take only the positive half
        fourier_freq = fftfreq(len(audio_hammed), 1 / audio_freq) # Fix values as Hz
